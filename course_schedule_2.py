        ...
        def course_can_be_completed(current_course):
            ...
            for course in dependency_map[current_course]:
                if course in visited:
                    can_be_completed &= course in courses_completed
                else:
                    course_can_be_completed(course)
                    can_be_completed &= course in courses_completed
                    
            if can_be_completed:
                courses_completed.add(current_course)
                result.append(current_course)
                
        for pre_req in prerequisites:
            dependency_map[pre_req[0]].append(pre_req[1])
            
        for course in range(numCourses):
            if course not in courses_completed:
                course_can_be_completed(course)
            
        if numCourses == len(courses_completed):
            return result
        else:
            return []
